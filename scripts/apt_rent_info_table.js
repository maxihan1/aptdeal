// /Users/maxi.moff/APT value/web/scripts/apt_rent_info_table.js (ES Modules 버전)
//
// 사용법:
//   node apt_rent_info_table.js [시작지역인덱스] [시작년월] [종료년월] [배치크기]
//
// 매개변수:
...
//
// 예시:
//   node apt_rent_info_table.js                    # 전체 처리
//   node apt_rent_info_table.js 100                # 100번째 지역부터 처리
//   node apt_rent_info_table.js 0 202401 202412    # 2024년 1월~12월만 처리
//   node apt_rent_info_table.js 50 202501 202507   # 50번째 지역부터 2025년 1월~7월 처리
...
const APT_RENT_API_BASE_URL = 'https://apis.data.go.kr/1613000/RTMSDataSvcAptRent/getRTMSDataSvcAptRent';
...
async function fetchAllPages(region, yearMonth) {
    ...
    while (true) {
        const apiUrl = `${APT_RENT_API_BASE_URL}?LAWD_CD=${region.lawdCd}&DEAL_YMD=${yearMonth}&numOfRows=${numOfRows}&pageNo=${pageNo}&serviceKey=${API_KEY}`;
...
// 데이터 적재
async function ingestAptRentData() {
    if (!API_KEY) {
    ...
    try {
                ...
                for (const yearMonth of yearMonths) {
                    ...
                    try {
                        ...
                        for (const item of items) {
                            try {
                                const [result] = await connection.execute(
                                    `INSERT IGNORE INTO apt_rent_info (
                                        aptNm, buildYear, contractTerm, contractType, dealDay, dealMonth,
                                        dealYear, deposit, excluUseAr, floor, jibun, monthlyRent,
                        ...
                        if (insertCount > 0) {
                            totalSuccessCount++;
                            console.log(`    ✅ ${yearMonth}: ${insertCount}개 전월세 데이터 삽입됨 (총 ${items.length}개 중)`);
                        } else {
        ...
        console.log('아파트 전월세 데이터 적재 스크립트가 완료되었습니다.');
        ...
        console.error('아파트 전월세 데이터 적재 스크립트가 실패했습니다.');
        process.exit(1);
    } finally {
        if (pool) {
            await pool.end();
