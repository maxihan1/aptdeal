...
async function batchInsert(rows, batchSize = 100) {
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    const { data, error } = await supabase.from('apt_rents').insert(batch);
...
function mapRentItems(items, lawdCd, yyyymm, pageNo, sido, sigungu) {
  const rows = items
    .filter(item => (item.dealYear || item.년) && (item.dealMonth || item.월) && (item.dealDay || item.일))
    .map((item, idx) => ({
      id: `${lawdCd}-${yyyymm}-${pageNo}-${item.일련번호 || item.rnum || idx}`,
      region: String(`${sido || ''} ${sigungu || ''} ${item.법정동 || item.umdNm || ''}`.trim()),
      address: String(item.도로명 || item.지번 || item.jibun || ''),
      area: Number(item.전용면적 || item.excluUseAr || 0),
      deposit: item.deposit || item.보증금액 || item.rentGtn ? Number(String(item.deposit || item.보증금액 || item.rentGtn).replace(/,/g, '')) : null,
      monthlyrent: item.monthlyRent || item.월세금액 || item.rentFee ? Number(String(item.monthlyRent || item.월세금액 || item.rentFee).replace(/,/g, '')) : null,
      contracttype: String(item.contractType || item.임대구분 || item.rentGbn || ''),
      ...
      tradetype: String(item.임대구분 || item.rentGbn || item.tradeType || ''),
...
(async () => {
  ...
  // sido, sigungu는 지역코드에서 역매핑 필요. 여기서는 빈 문자열로 둠(추후 필요시 regionsLawd에서 역매핑 구현)
  for (const lawdCd of codes) {
    for (let year = START_YEAR; year <= END_YEAR; year++) {
      for (let month = 1; month <= 12; month++) {
        ...
        try {
          console.log(`[${lawdCd}] ${yyyymm} 전월세 데이터 요청`);
          let allRows = [];
          let pageNo = 1;
          while (true) {
            const rents = await fetchAllAptRents({ ...params, pageNo });
            if (!rents || rents.length === 0) break;
            const rows = mapRentItems(rents, lawdCd, yyyymm, pageNo, '', '');
            allRows = allRows.concat(rows);
            if (rents.length < 100) break;
            pageNo++;
          }
          console.log(`[${lawdCd}] ${yyyymm} 적재 rows:`, allRows.length);
          await batchInsert(allRows, 100);
        } catch (e) {
