// /Users/maxi.moff/APT value/web/scripts/apt_basic_info_table.js (ES Modules 버전)
//
// 사용법:
//   node apt_basic_info_table.js [시작인덱스] [배치크기] [기존데이터건너뛰기]
//
// 매개변수:
...
//
// 예시:
//   node apt_basic_info_table.js                    # 처음 100개 처리
//   node apt_basic_info_table.js 100                # 100번째부터 100개 처리
//   node apt_basic_info_table.js 100 50             # 100번째부터 50개 처리
//   node apt_basic_info_table.js 100 50 true        # 100번째부터 50개 처리, 기존 데이터 건너뛰기
//
// 중간에 끊어진 경우 이어서 처리:
//   node apt_basic_info_table.js 0 1000 true        # 처음부터 1000개 처리, 기존 데이터 건너뛰기
...
const APT_BASIS_INFO_API_BASE_URL = 'https://apis.data.go.kr/1613000/AptBasisInfoServiceV4/getAphusBassInfoV4';
...
async function ingestAptBasisInfoData() {
    if (!API_KEY) {
    ...
    try {
        ...
        const [existingCodes] = await connection.execute(
            'SELECT kaptCode FROM apt_basis_info'
        );
        ...
        for (let i = 0; i < kaptCodes.length; i++) {
            ...
            const apiUrl = `${APT_BASIS_INFO_API_BASE_URL}?serviceKey=${API_KEY}&kaptCode=${kaptCode}`;
            ...
            try {
                ...
                const [result] = await connection.execute(
                    `INSERT IGNORE INTO apt_basis_info (
        ...
        console.log('아파트 기본 정보 적재 스크립트가 완료되었습니다.');
        ...
        console.error('아파트 기본 정보 적재 스크립트가 실패했습니다.');
        process.exit(1);
    } finally {
        if (pool) {
            await pool.end();
