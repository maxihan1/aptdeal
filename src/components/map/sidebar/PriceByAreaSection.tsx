...
interface PriceByAreaItem {
    area: string;      // "59㎡", "84㎡" 등
    avgPrice: number;
    count: number;
}

interface PriceByAreaSectionProps {
    data: PriceByAreaItem[];
}
...
export default function PriceByAreaSection({ data }: PriceByAreaSectionProps) {
    ...
    return (
        <div className="space-y-2">
            <h4 className="font-semibold text-sm flex items-center gap-1.5">
                ...
                면적별 평균가 <span className="text-xs font-normal text-muted-foreground">(최근 1년)</span>
            </h4>
                ...
                {sortedData.map((item, idx) => (
                    ...
                    >
                        ...
                        <div className="font-semibold text-sm text-primary">
                            {formatPriceShort(item.avgPrice)}
                        </div>
                    </div>
                ))}
            </div>
